...
public class PayActivity extends FragmentActivity {
		...
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					...
					setResult(Config.PAY_SUCCESS, intent);
					finish();
				} else {
	...
	private String cast;
	private Record payInfo;
	private float price;
	private ExternalFragment fragment;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		payInfo= (Record) bundle.getSerializable("payInfo");
		movie=payInfo.getTitle();
		cast=payInfo.getSummary();
		price=payInfo.getPrice();
		Log.i("payActivity","启动支付---"+movie+"-----"+cast+"-----");
		fragment=new ExternalFragment();
		fragment.setInfo(movie,cast,price+"");
		getSupportFragmentManager().beginTransaction().replace(R.id.fl_pay_container,fragment).commit();
//        fragment=findViewById(R.id.fragment);
	...
	public void pay(View v) {
		...
		// 订单
		double price=0.02;
		String orderInfo = getOrderInfo(movie, cast, price+"");
		Log.i("payActivity","---"+movie+"-----"+cast+"-----"+price);
